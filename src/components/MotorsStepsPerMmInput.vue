...
<template>
	<div>
		<b-form-input ref="stepsPerMm" v-model.number="drive.steps_per_mm" v-preset="presetDrive.steps_per_mm" title="Motor steps per mm (M92)" min="1" type="number" step="any" required />
		<b-popover :target="inputElement" :show.sync="popoverShown" placement="right" title="Calculate steps per mm" triggers="focus">
			<b-form>
				...
				<b-form-group v-if="driveType != 'extruder'" horizontal label="Drive type:" label-cols="5">
				...
				<template v-if="driveType == 'belt'">
					<b-form-group horizontal label="Preset:" label-cols="5">
						<b-select v-model="beltPreset">
							<option v-for="option in belt.presets" :value="option.value" v-text="option.text" />
							<option value="custom">Custom</option>
						</b-select>
					</b-form-group>
					<b-form-group horizontal label="Belt pitch:" label-cols="5">
						<b-input-group append="mm">
							<b-form-input v-model.number="belt.pitch" min="0.1" type="number" step="any" required />
						</b-input-group>
					</b-form-group>
					<b-form-group horizontal label="Pulley teeth:" label-cols="5">
						<b-form-input v-model.number="belt.pulleyTeeth" min="1" type="number" step="1" required />
					</b-form-group>
				</template>
					...
					<b-form-group horizontal label="Preset:" label-cols="5">
						<b-select v-model="leadscrewPreset">
							<optgroup v-for="(group, name) in leadscrew.presets" :label="name">
								<option v-for="option in group" :value="option.value" v-text="option.text" />
							</optgroup>
							<option value="custom">Custom</option>
						</b-select>
					</b-form-group>
					<b-form-group horizontal label="Pitch:" label-cols="5">
						<b-input-group append="mm">
							<b-form-input v-model.number="leadscrew.pitch" type="number" step="any" required />
						</b-input-group>
					</b-form-group>
					<b-form-group horizontal label="Gear ratio:" label-cols="5">
						<table>
							<tr>
								<td>
									<b-form-input v-model.number="leadscrew.ratio1" class="no-spinners" type="number" step="any" required />
								</td>
								<td>:</td>
								<td>
									<b-form-input v-model.number="leadscrew.ratio2" class="no-spinners" type="number" step="any" required />
								</td>
							</tr>
						</table>
					</b-form-group>
				...
				<template v-else>
					<b-form-group horizontal label="Hob diameter:" label-cols="5">
						<b-input-group append="mm" class="mt-2">
							<b-form-input v-model.number="extruder.hob" type="number" step="any" required />
						</b-input-group>
					</b-form-group>
					<b-form-group horizontal label="Gear ratio:" label-cols="5">
						<table>
							<tr>
								<td>
									<b-form-input v-model.number="extruder.ratio1" class="no-spinners" type="number" step="any" required />
								</td>
								<td>:</td>
								<td>
									<b-form-input v-model.number="extruder.ratio2" class="no-spinners" type="number" step="any" required />
								</td>
							</tr>
						</table>
					</b-form-group>
					<b-form-group horizontal label="Amount extruded:" label-cols="5">
						<b-input-group append="mm" class="mt-2">
							<b-form-input v-model.number="extruder.amountExtruded" type="number" step="any" required />
						</b-input-group>
					</b-form-group>
					<b-form-group horizontal label="Actually extruded:" label-cols="5">
						<b-input-group append="mm" class="mt-2">
							<b-form-input v-model.number="extruder.actuallyExtruded" type="number" step="any" required />
						</b-input-group>
					</b-form-group>
				</template>
				<b-card bg-variant="light">
					...
					<h3 :class="{ 'text-danger' : !isValid }">
						{{ isValid ? calculatedSteps.toFixed(3) : "error" }}
...
export default {
	...
	methods: {
		apply() {
			this.drive.steps_per_mm = this.calculatedSteps;
